...
# loo 버전에 따라 model 열이 이미 존재할 수 있다.
if (!"model" %in% names(loo_pairwise)) {
  loo_pairwise <- tibble::rownames_to_column(loo_pairwise, var = "model")
} else {
  rownames(loo_pairwise) <- NULL
}
...
# ---------------------------
# 4-5) 발생 기록 조건부 예측평균: 빠른 해석적 계산
# ---------------------------
# brms::fitted()는 truncated discrete 모형에서 posterior_epred()를 호출하며,
# 일반적인 count 모형보다 매우 오래 걸릴 수 있다. 여기서는 log-link의 mu와
# 음이항 shape 사후표본을 추출한 뒤, Y>0 조건부 평균을 닫힌식으로 계산한다.
#
# Zero-truncated Poisson:
#   E(Y | Y>0, mu) = mu / {1 - exp(-mu)}
# Zero-truncated Negative Binomial (brms의 mean-shape 모수화):
#   P(Y=0 | mu, shape) = {shape / (shape + mu)}^shape
#   E(Y | Y>0, mu, shape) = mu / {1 - P(Y=0)}

N_PRED_DRAWS <- 1000L  # 빠른 확인은 500L, 최종 요약은 1000~2000L 권장
...
summarise_epred_matrix <- function(epred, prefix) {
  q <- apply(
    epred,
    2,
    stats::quantile,
  ...
  out <- data.frame(
    pred_mean = colMeans(epred, na.rm = TRUE),
    pred_sd = apply(epred, 2, stats::sd, na.rm = TRUE),
    pred_lower = q[1, ],
    pred_median = q[2, ],
    pred_upper = q[3, ],
    check.names = FALSE
  ...
  names(out) <- paste0(prefix, c(
    "_pred_mean", "_pred_sd", "_pred_lower", "_pred_median", "_pred_upper"
  ))
  out
}

make_ztp_prediction_table <- function(fit, prefix = "ztp", n_draws = N_PRED_DRAWS) {
  draw_ids <- make_draw_ids(fit, n_draws)

  # CAR 공간효과를 포함한 parent Poisson 평균 mu = exp(eta)
  ...
  # 1 - exp(-mu)를 -expm1(-mu)로 계산해 작은 mu에서도 수치적으로 안정화
  denom <- pmax(-expm1(-mu), .Machine$double.eps)
  epred <- mu / denom

  summarise_epred_matrix(epred, prefix)
}

make_ztnb_prediction_table <- function(fit, prefix = "ztnb", n_draws = N_PRED_DRAWS) {
  draw_ids <- make_draw_ids(fit, n_draws)

  # CAR 공간효과를 포함한 parent NB 평균 mu = exp(eta)
  mu <- brms::posterior_linpred(
    fit,
    transform = TRUE,
    draw_ids = draw_ids,
  ...
  # 동일한 draw_ids에 대응하는 음이항 shape 사후표본
  shape <- brms::posterior_linpred(
    fit,
    dpar = "shape",
    transform = TRUE,
    draw_ids = draw_ids,
  ...
  # brms 버전에 따라 shape가 1열만 반환되면 관측치 수만큼 확장
  if (ncol(shape) == 1L && ncol(mu) > 1L) {
    shape <- matrix(shape[, 1], nrow = nrow(mu), ncol = ncol(mu))
  }
  if (!all(dim(shape) == dim(mu))) {
    stop(
      "mu와 shape 사후표본의 차원이 다릅니다: mu=",
      paste(dim(mu), collapse = "x"),
      ", shape=", paste(dim(shape), collapse = "x")
    )
  }

  log_p0 <- shape * (log(shape) - log(shape + mu))
  p0 <- exp(log_p0)
  denom <- pmax(1 - p0, .Machine$double.eps)
  epred <- mu / denom

  summarise_epred_matrix(epred, prefix)
}

message("[예측] zero-truncated Poisson 조건부 평균 계산")
pred_ztp <- make_ztp_prediction_table(fit_ztp, "ztp")

message("[예측] zero-truncated Negative-binomial 조건부 평균 계산")
pred_ztnb <- make_ztnb_prediction_table(fit_ztnb, "ztnb")
...
find_bad_parameters <- function(
  ...
  # brmsfit → posterior draws
  draws <- posterior::as_draws_array(fit)
  
  # 기본 요약에 rhat, ess_bulk, ess_tail이 포함됨
  result <- posterior::summarise_draws(draws) |>
    as.data.frame()
  ...
  # NA가 아닌 진단값 중 기준을 위반한 모수만 선택
  bad_idx <-
    (!is.na(result$rhat) &
       result$rhat > rhat_cutoff) |
    (!is.na(result$ess_bulk) &
       result$ess_bulk < ess_cutoff) |
    (!is.na(result$ess_tail) &
       result$ess_tail < ess_cutoff)
  ...
  if (nrow(bad) == 0) {
    cat("설정한 기준을 위반한 모수가 없습니다.\n")
  } else {
    cat("문제가 있는 모수 수:", nrow(bad), "\n")
    print(
      utils::head(bad, 50),
      row.names = FALSE
    )
...
bad_ztp <- find_bad_parameters(
  fit_ztp,
  "Zero-truncated Poisson CAR"
)

bad_ztnb <- find_bad_parameters(
  fit_ztnb,
  "Zero-truncated Negative-binomial CAR"
)


check_sampler <- function(fit, max_treedepth = 15) {
...
check_sampler(fit_ztnb)

diag_ztnb <- posterior::summarise_draws(
  posterior::as_draws_array(fit_ztnb)
) |>
  as.data.frame()

max(diag_ztnb$rhat, na.rm = TRUE)
min(diag_ztnb$ess_bulk, na.rm = TRUE)
min(diag_ztnb$ess_tail, na.rm = TRUE)

#-----------------------------------------------

# LOO 관측치별 진단
fit_ztnb$criteria$loo
...
# 3. IRR 표
irr_ztnb <- data.frame(
  variable = rownames(fe_ztnb),
  Estimate = fe_ztnb[, "Estimate"],
  Est_Error = fe_ztnb[, "Est.Error"],
  Lower_95 = fe_ztnb[, "Q2.5"],
  Upper_95 = fe_ztnb[, "Q97.5"],
  IRR = exp(fe_ztnb[, "Estimate"]),
  IRR_Lower_95 = exp(fe_ztnb[, "Q2.5"]),
  IRR_Upper_95 = exp(fe_ztnb[, "Q97.5"]),
  credible = (
    fe_ztnb[, "Q2.5"] > 0 |
      fe_ztnb[, "Q97.5"] < 0
  )
...
# 5. 조건부 효과 그림
ce_ztnb <- brms::conditional_effects(
  fit_ztnb,
  effects = "species:sum_sun_z"
)
plot(ce_ztnb, points = TRUE)
#
ce_ztnb_lp <- brms::conditional_effects(
  fit_ztnb,
  effects = "species:sum_sun_z",
